<template>
    <div>
        ...
        <form
            action="#"
            @submit.prevent="edit ? updateBranch(form.id) : createBranch()"
            v-show="showForm"
            @keydown="form.errors.clear($event.target.name)"
        >
            <div
                class="form-group"
                v-bind:class="{ 'has-error': form.errors.any() }"
            >
        ...
        >
            <template slot="options" slot-scope="props">
                <button
                    @click="editBranch(props.id)"
                    class="btn btn-primary btn-xs"
                >
                    Edit
                </button>
                <button
                    @click="deleteBranch(props.id)"
...
export default {
    ...
    methods: {
        ...
        editBranch(id) {
            this.form.get("api/branches/" + id).then((data) => {
        ...
        updateBranch(id) {
            this.form.put("api/branches/" + id).then(() => {
                this.edit = false;
                this.showForm = false;
                this.refresh();
            });
